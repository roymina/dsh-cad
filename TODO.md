...
- [x] 3.1 消除 `any` 与私有字段依赖：使用库公开类型（`CadDocument`、`Entity`、`Layer`、`BlockRecord`），改用稳定 `objectName`、`Color.index` / `getRgb()`、`TextEntity.value` / `height`、`Circle.radius` 等公开接口；建立规范化层与显式判别联合 `{ ok: true, ... } | { ok: false, error: ... }`。
