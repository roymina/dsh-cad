...
- [ ] 2.1 评估并封装 `@node-projects/acad-ts` 自带的 `SvgWriter`（含 Arc、Ellipse、Hatch、Insert、Dimension、颜色、线型、文字转换）；不满足则建立按实体类型分派的 renderer，而非在单个 `map` 中继续追加条件。
- [ ] 2.2 支持 Insert 展开：平移、旋转、缩放、阵列、嵌套块与循环引用深度限制。
...
- [ ] 2.10 分别统计源实体、实际 SVG 图元、被跳过实体与块展开后的实体（对齐 `NEXT_PLAN.md` §3.2 建议输出结构）。
- [ ] 2.11 增加视觉回归测试（SVG 结构快照、受控 PNG golden/感知差异），并同时校验 unsupported 统计。
...
- [ ] 3.6 增加资源限制：总顶点数、单实体最大顶点数、块展开实例数与最大递归深度、SVG 最大字节数、PNG 最大宽/高与总像素数、单条文本与最终 JSON/CSV 最大字节数。
- [ ] 3.7 在读取、解析、规范化、渲染、写入各阶段之间检查 `AbortSignal`。
- [ ] 3.8 输入路径权限：增加 `allowedInputRoots`（默认限制到工作区）、canonical path 归属检查、区分 `ENOENT` / `EACCES` / 非法路径 / 读取失败、明确符号链接策略并补测试。
- [ ] 3.9 原子无覆盖输出：使用 `flag: 'wx'` 或同目录临时文件 + 原子 rename；默认输出名增加短哈希/唯一后缀；失败时清理本次调用产生的临时文件；可增加输出目录容量与保留期限策略。
- [ ] 3.10 SVG 安全：校验 `background` 颜色格式（`transparent`、受控命名色、`#RGB`、`#RRGGBB`、`#RRGGBBAA`）；统一转义 XML 属性值；删除 XML 1.0 非法控制字符；禁止 `url(...)` 外部资源网络访问。
- [ ] 3.11 CSV 安全：对 `=`、`+`、`-`、`@` 开头单元格做公式注入防护；增加 UTF-8 BOM 选项；限制最大行数、单元格长度与输出字节数。
- [ ] 3.12 性能优化：基于 canonical path + mtime + size 的有界 LRU 缓存（最大条目/内存估算/失效策略），并为大图提供一次完成多项分析的组合接口。

## 阶段四：产品化（Harness / 发布 / 文档）

- [ ] 4.1 为成功与失败结果建立稳定输出 schema；文档化所有错误码（`NEXT_PLAN.md` §9）与可恢复建议。
...
- [ ] 4.3 工具描述中列出合法实体类型、Layout 范围、预览局限与转换数据损失风险。
...
- [ ] 4.7 发布流程：增加 `prepack`（typecheck + test + build）、CI（typecheck/test/build/pack dry-run/生产依赖审计）、`exports`/Node `engines`/repository/keywords/publishConfig/sideEffects 声明，并明确 `lib` 是否提交。
...
- [ ] 4.9 完善 README：配置项/默认值/范围/安全影响、三工具完整调用与返回示例、错误码及恢复、DWG/DXF 版本支持矩阵、预览实体支持矩阵、DWG→DXF 保真限制与警告语义、统计范围、路径与输出策略、性能/超时/资源上限。

## 阶段五：测试计划（随各阶段同步推进）

- [ ] 5.1 单元测试：点/bounds 规范化、图层 flags/颜色/线型/可见性、稳定实体类型映射、closed/bulge polyline、XML 文本与属性转义、CSV 引号/换行/中文/公式注入、limit/width/颜色等边界参数、输出路径与文件名规则。
...
- [ ] 5.3 语义回归测试：转换前后比较全部文本值、模型/纸空间实体统计、图层及 flags、块定义与引用、关键几何坐标与 bounds、单位/版本/code page、解析/写入警告。
- [ ] 5.4 视觉回归测试：小型合成图 SVG 结构快照、关键样本受控 PNG golden/感知差异、单独验证旋转文字/颜色/线型/闭合折线/bulge/Insert 变换/Hatch，并同时检查 unsupported 统计。
...
- [ ] 6.2 距离、总长度、周长与面积统计。
- [ ] 6.3 闭合轮廓识别。
- [ ] 6.4 重复实体、零长度线、非法半径、开口轮廓检查。
- [ ] 6.5 图层使用率与空图层检测。
- [ ] 6.6 块使用、嵌套与重复定义分析。
- [ ] 6.7 Xref、图片、字体与缺失资源报告。
- [ ] 6.8 单位归一化与坐标转换。
- [ ] 6.9 图纸差异比较。
- [ ] 6.10 面向 LLM 的工程摘要、异常摘要与问答上下文索引。
