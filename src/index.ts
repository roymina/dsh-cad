import { readFile, lstat, mkdir, realpath, writeFile } from 'node:fs/promises'
import path from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import Schema from '@deepseek-ai/schemastery'
import { defineTool } from '@deepseek-ai/dsh-tools'
import { DwgReader, DxfReader, DxfWriter } from '@node-projects/acad-ts'
...
function extract(document: CadDocument, section: string, layers: string[] | undefined, entityTypes: string[] | undefined, limit: number) {
  ...
  const records = source.slice(0, limit).map((item: any) => section === 'layers'
    ? { name: item.name, isOn: item.isOn !== false, isFrozen: Boolean(item.isFrozen), colorIndex: item.color?._color ?? item._color?._color ?? null }
...
function makeSvg(document: CadDocument, selectedLayers?: string[], background = 'white') {
  ...
  const content = drawing.map(entity => {
    const color = '#202020'
    const kind = entityName(entity)
    if (kind === 'Line') { const a = point(entity.startPoint); const b = point(entity.endPoint); return a && b ? `<line x1="${a.x}" y1="${-a.y}" x2="${b.x}" y2="${-b.y}" stroke="${color}"/>` : '' }
    if (kind === 'LwPolyline' || kind === 'Polyline2D' || kind === 'Polyline3D') { const values = drawingPoints(entity).map(p => `${p.x},${-p.y}`).join(' '); return values ? `<polyline points="${values}" fill="none" stroke="${color}"/>` : '' }
    if (kind === 'Circle') { const center = point(entity.center); const radius = Number(entity.radius ?? entity._radius); return center && Number.isFinite(radius) ? `<circle cx="${center.x}" cy="${-center.y}" r="${radius}" fill="none" stroke="${color}"/>` : '' }
    if (kind === 'TextEntity' || kind === 'MText') { const at = point(entity.insertPoint); const value = String(entity.value ?? entity._value ?? ''); const size = Number(entity.height ?? entity._height ?? 2.5); return at ? `<text x="${at.x}" y="${-at.y}" font-size="${size}" fill="${color}">${escapeXml(value)}</text>` : '' }
    return ''
  }).join('')
  const bg = background === 'transparent' ? '' : `<rect x="${minX}" y="${-maxY}" width="${width}" height="${height}" fill="${background}"/>`
  return { svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${minX} ${-maxY} ${width} ${height}">${bg}<g stroke-width="${Math.max(width, height) / 2500}">${content}</g></svg>`, bounds: { min: { x: minX, y: minY }, max: { x: maxX, y: maxY } }, entityCount: drawing.length }
}
...
export async function exportCad(args: { path: string; format: 'svg' | 'png' | 'dxf'; outputName?: string; layers?: string[]; width?: number; background?: string }, config: Config, signal?: AbortSignal) {
  ...
  try {
    ...
    if (args.format === 'dxf') {
      const chunks: string[] = []
      DxfWriter.writeToStream({ write: (value: string) => chunks.push(value) }, loaded.document as any)
    ...
    return { ok: true, format: args.format, outputPath: output, bounds: drawing.bounds, renderedEntityCount: drawing.entityCount, warnings: loaded.warnings }
  } catch (cause) {
