...
if (!parentPort) throw new Error('CAD parse worker requires parentPort.')

type ParseMessage = { format: 'dwg' | 'dxf'; buffer: ArrayBuffer }

parentPort.on('message', (message: ParseMessage) => {
  const warnings: string[] = []
  const notify = (_sender: unknown, event: unknown) => warnings.push(String(event && typeof event === 'object' && 'message' in event ? event.message : event))
  try {
