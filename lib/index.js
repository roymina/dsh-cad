...
async function renderPngIsolated(svg, mode, value, background, signal, timeoutMs = 30_000) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const finish = (callback) => { if (settled)
            return; settled = true; clearTimeout(timer); signal?.removeEventListener('abort', cancel); void worker.terminate(); callback(); };
        const cancel = () => finish(() => reject(new Error('CANCELLED')));
        const timer = setTimeout(() => finish(() => reject(new Error('TIMEOUT'))), timeoutMs);
        signal?.addEventListener('abort', cancel, { once: true });
        worker.on('message', (message) => message.error ? finish(() => reject(new Error(message.error))) : finish(() => resolve({ width: message.width, height: message.height, png: message.png })));
        worker.on('error', error => finish(() => reject(error)));
        worker.postMessage({ svg, mode, value, background });
    });
}
async function runCadOperation(operation, args, config, signal) {
    if (signal?.aborted)
    ...
    try {
        ...
        return await new Promise((resolve, reject) => {
            ...
            signal?.addEventListener('abort', cancel, { once: true });
            worker.on('message', (message) => message.error ? finish(() => reject(new Error(message.error))) : finish(() => resolve(message.result)));
            worker.on('error', error => {
                if (/Cannot find module|ERR_MODULE_NOT_FOUND/.test(error.message))
...
async function parseCadIsolated(format, buffer, signal, timeoutMs = 30_000) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        worker.on('message', (message) => {
            if (message.error)
                return finish(() => reject(new Error(message.error)));
            if (!message.buffer || !message.format)
                return finish(() => reject(new Error('WORKER_INVALID_RESULT')));
...
function entityName(entity) {
    ...
    return { LINE: 'Line', CIRCLE: 'Circle', ARC: 'Arc', ELLIPSE: 'Ellipse', SPLINE: 'Spline', HATCH: 'Hatch', INSERT: 'Insert', POINT: 'Point', SOLID: 'Solid', LEADER: 'Leader', DIMENSION: 'Dimension', TEXT: 'TextEntity', MTEXT: 'MText', ATTRIB: 'AttributeEntity', LWPOLYLINE: 'LwPolyline', POLYLINE: 'Polyline2D', POLYLINE3D: 'Polyline3D' }[objectName] ?? objectName;
}
...
const unitToMillimeters = { 1: 25.4, 2: 304.8, 4: 1, 5: 10, 6: 1000, 7: 1_000_000 };
function normalizeBounds(boundsValue, units) {
    if (!boundsValue)
        return null;
...
function resourceLimitError(document, config) {
    const all = entities(document);
    if (all.length > config.maxEntities)
        return error('ENTITY_LIMIT_EXCEEDED', 'The drawing exceeds the configured entity limit.', { entityCount: all.length, maxEntities: config.maxEntities });
    ...
    for (const entity of all) {
        ...
        if (count > maxEntityVertices)
            return error('GEOMETRY_LIMIT_EXCEEDED', 'An entity exceeds the configured vertex limit.', { handle: entity.handle, vertices: count, maxEntityVertices });
        totalVertices += count;
    }
    if (totalVertices > maxTotalVertices)
        return error('GEOMETRY_LIMIT_EXCEEDED', 'The drawing exceeds the configured total vertex limit.', { totalVertices, maxTotalVertices });
    return undefined;
}
...
function geometryMetrics(document) {
    let totalLength = 0;
    let perimeter = 0;
    let area = 0;
    for (const entity of entities(document)) {
        ...
        if (kind === 'Line') {
            ...
            const points = drawingPoints(entity);
            let length = 0;
            for (let i = 1; i < points.length; i++)
                length += Math.hypot(points[i].x - points[i - 1].x, points[i].y - points[i - 1].y);
            if (entity.isClosed && points.length > 1) {
                length += Math.hypot(points[0].x - points.at(-1).x, points[0].y - points.at(-1).y);
                perimeter += length;
            }
            totalLength += length;
            if (entity.isClosed && points.length > 2)
                area += Math.abs(points.reduce((sum, current, i) => { const next = points[(i + 1) % points.length]; return sum + current.x * next.y - next.x * current.y; }, 0)) / 2;
        }
    }
    return { totalLength, perimeter, area };
}
function layerUsage(document) {
    const counts = {};
    for (const entity of entities(document))
        counts[entityLayer(entity)] = (counts[entityLayer(entity)] ?? 0) + 1;
...
function qualityChecks(document) {
    ...
    for (const entity of entities(document)) {
        if (handles.has(entity.handle))
            duplicateHandles.push(`0x${entity.handle.toString(16).toUpperCase()}`);
        else
        ...
        if (kind === 'Circle' || kind === 'Arc')
            if (!Number.isFinite(Number(entity.radius)) || Number(entity.radius) <= 0)
                invalidRadii++;
        if (['LwPolyline', 'Polyline2D', 'Polyline3D'].includes(kind)) {
            if (!entity.isClosed)
                openPolylines++;
            else
                closedContours++;
...
async function loadCad(input, config, signal) {
    ...
    try {
        ...
        if (causeCode(cause) === 'EACCES' || causeCode(cause) === 'EPERM')
    ...
    try {
        ...
        const notify = (_sender, event) => warnings.push({ code: 'PARSER_WARNING', message: String(event && typeof event === 'object' && 'message' in event ? event.message : event) });
        const isolated = config.workerMode ? undefined : await parseCadIsolated(format, buffer, signal, config.maxWorkerTimeMs ?? 30_000);
        ...
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'CAD parsing exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
...
function inspect(document, inputPath, format, warnings, maxWarningSamples) {
    ...
    return success({
        ...
        blocks: blocks(document).map(block => ({ name: block.name, entityCount: Array.from(block.entities ?? []).length, nestedBlocks: Array.from(block.entities ?? []).filter(entity => entityName(entity) === 'Insert').map(entity => entity.block?.name).filter(Boolean) })),
...
function entityRecord(entity) {
    ...
    if (kind === 'Insert')
        return { ...base, block: entity.block?.name ?? null, position: point(entity.insertPoint), rotation: entity.rotation ?? 0, scale: { x: entity.xScale ?? 1, y: entity.yScale ?? 1, z: entity.zScale ?? 1 }, attributes: Array.from(entity.attributes ?? []).map(attribute => ({ tag: attribute.tag, text: attribute.value, position: point(attribute.insertPoint) })) };
    return base;
}
function extract(document, section, layers, entityTypes, limit, offset = 0, search, handle, window, nearest) {
    const all = entities(document);
    ...
    const filtered = all.filter(entity => {
        ...
        if (window) {
            ...
            inWindow = Boolean(box && box.max.x >= window.minX && box.min.x <= window.maxX && box.max.y >= window.minY && box.min.y <= window.maxY);
        }
    ...
    let source = section === 'texts' ? filtered.filter(entity => ['TextEntity', 'MText', 'AttributeEntity'].includes(entityName(entity)))
        : section === 'layers' ? Array.from(document.layers ?? [])
            : section === 'blocks' ? blocks(document)
                : filtered;
    if (nearest)
        source = [...source].sort((a, b) => { const pa = point(a.insertPoint ?? a.location ?? a.center) ?? { x: 0, y: 0 }; const pb = point(b.insertPoint ?? b.location ?? b.center) ?? { x: 0, y: 0 }; return Math.hypot(pa.x - nearest.x, pa.y - nearest.y) - Math.hypot(pb.x - nearest.x, pb.y - nearest.y); });
    const records = source.slice(offset, offset + limit).map(item => section === 'layers'
        ...
        : section === 'blocks' ? { name: item.name, entityCount: Array.from(item.entities ?? []).length }
            : entityRecord(item));
    return success({ section, offset, total: source.length, returned: records.length, truncated: offset + records.length < source.length, records });
}
...
function hasCircularBlockReference(block, blockStack = new Set()) {
    ...
    return childEntities.some(entity => entityName(entity) === 'Insert' && entity.block ? hasCircularBlockReference(entity.block, nextStack) : false);
}
function expandInsert(insert, maxDepth, maxInstances, depth = 0, blockStack = new Set()) {
    const blockName = String(insert.block?.name ?? '');
    if (!blockName || !insert.block || depth >= maxDepth || blockStack.has(blockName) || hasCircularBlockReference(insert.block))
        return [];
    const nextStack = new Set(blockStack).add(blockName);
    ...
    for (let index = 0; index < total; index++) {
        ...
        if (!insertPoint || !instance.explode)
            return expanded;
...
async function inspectCadCore(pathValue, config, signal) {
...
async function compareCadCore(firstPath, secondPath, config, signal) {
    ...
    if (isErrorResult(second))
        return second;
    const left = semanticSnapshot(first.document);
    const right = semanticSnapshot(second.document);
...
async function extractCadCore(args, config, signal) {
    ...
    if (args.summary)
        return success({ section: result.section, total: result.total, offset: result.offset, returned: result.returned, truncated: result.truncated });
    if (!args.saveAs)
        return result;
    try {
...
async function exportCadCore(args, config, signal) {
    const invalid = invalidPath(args.path);
    if (invalid)
        return invalid;
    if (args.outputName !== undefined && !validOutputName(args.outputName))
    ...
    try {
        ...
        if (args.format === 'dxf') {
            ...
            return success({ format: 'dxf', outputPath: output, ...(await outputMetadata(output)), conversionValidation, lossRisk,
                unpreservedObjectTypes: conversionValidation.unpreservedObjectTypes,
                warnings: summarizeWarnings([...loaded.warnings, ...conversionWarnings, ...converted.warnings], config.maxWarningSamples ?? 50),
            });
        }
        ...
        if (args.format === 'svg') {
            ...
            try {
                ...
                if (cause instanceof Error && /Cannot find module|ERR_MODULE_NOT_FOUND/.test(cause.message)) {
                    ...
                    rendered = { width: fallback.width, height: fallback.height, png: fallback.asPng() };
                }
                else
                    throw cause;
            }
        ...
        return success({ format: args.format, outputPath: output, bounds: drawing.bounds,
        ...
        if (cause instanceof Error && cause.message === 'CANCELLED')
            return error('CANCELLED', 'Operation was cancelled.');
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'Rendering exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
...
export async function inspectCad(pathValue, config, signal) {
    if (config.workerMode)
        return inspectCadCore(pathValue, config, signal);
    try {
        ...
        if (cause instanceof Error && cause.message === 'CANCELLED')
            return error('CANCELLED', 'Operation was cancelled.');
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'CAD inspection exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
        return error('PARSE_FAILED', 'The isolated CAD operation failed.', { message: cause instanceof Error ? cause.message : String(cause) });
    }
}
export async function compareCad(firstPath, secondPath, config, signal) {
    if (config.workerMode)
        return compareCadCore(firstPath, secondPath, config, signal);
    try {
        ...
        if (cause instanceof Error && cause.message === 'CANCELLED')
            return error('CANCELLED', 'Operation was cancelled.');
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'CAD comparison exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
        return error('PARSE_FAILED', 'The isolated CAD operation failed.', { message: cause instanceof Error ? cause.message : String(cause) });
    }
}
export async function extractCad(args, config, signal) {
    if (config.workerMode)
        return extractCadCore(args, config, signal);
    try {
        ...
        if (cause instanceof Error && cause.message === 'CANCELLED')
            return error('CANCELLED', 'Operation was cancelled.');
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'CAD extraction exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
        return error('PARSE_FAILED', 'The isolated CAD operation failed.', { message: cause instanceof Error ? cause.message : String(cause) });
    }
}
export async function exportCad(args, config, signal) {
    if (config.workerMode)
        return exportCadCore(args, config, signal);
    try {
        ...
        if (cause instanceof Error && cause.message === 'CANCELLED')
            return error('CANCELLED', 'Operation was cancelled.');
        if (cause instanceof Error && cause.message === 'TIMEOUT')
            return error('TIMEOUT', 'CAD export exceeded the worker time limit.', { maxWorkerTimeMs: config.maxWorkerTimeMs ?? 30_000 });
...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) { return asJson(await extractCad(args, config, exec.signal)); },
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) { return asJson(await exportCad(args, config, exec.signal)); },
    }));
}
